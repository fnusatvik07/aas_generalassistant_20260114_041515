# ui_user_13284a42/main.py
"""
General Assistant - Auto-generated FastAPI service
...
CORS_ORIGINS = [
    ...
    f"http://localhost:8046",
    f"http://localhost:8047", 
    f"http://localhost:8048",
    f"http://127.0.0.1:8046",
    f"http://127.0.0.1:8047",
...
AGENT_CONFIG = {
    ...
    "system_prompt": """You are a helpful AI assistant. User's request: Automatically respond to customer inquiries, escalate complex issues, and track resolution times.
...
async def root():
    return {
        "message": "Welcome to General Assistant",
        "role": "General Purpose Assistant",
        "agent_id": "ui_user_13284a42",
        "endpoints": [
...
async def get_frontend_info():
    ...
    return {
        "backend_url": f"http://localhost:8045",
        "frontend_url": f"http://localhost:8046",
        "cors_enabled": True,
...
async def stream_agent_progress(request: QueryRequest) -> AsyncGenerator[str, None]:
    ...
    try:
        ...
        yield create_event("progress", {
            "message": "[AGENT] Starting General Assistant...",
...
async def query_agent(request: QueryRequest):
    """
    Send a query/task to the General Assistant (non-streaming)
    """
    try:
        # Create Claude Agent SDK options
        options = ClaudeAgentOptions(
...
if __name__ == "__main__":
    ...
    print(f"[INFO] General Assistant Backend Starting...")
    print(f"[BACKEND] Backend URL: http://localhost:8045")
    print(f"[FRONTEND] Frontend URL: http://localhost:8046")  
    print(f"[CORS] CORS: Enabled for frontend connections")
    print(f"[DOCS] API Docs: http://localhost:8045/docs")
    print(f"[HEALTH] Health: http://localhost:8045/health")
